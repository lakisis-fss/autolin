...
# 프로젝트 루트 경로를 sys.path에 추가하여 모듈 임포트 문제를 해결합니다.
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

# Import our CV reader
...
def main():
    print("=" * 60)
    print("      리니지 클래식 무개입 100% 자동 메모리 오프셋 복구 엔진")
    print("=" * 60)
    
    print("[*] 1단계: 컴퓨터 비전(OCR) 엔진을 가동하여 화면을 캡처 중...")
    try:
        reader = CVStateReader(debug=False)
        cv_state = reader.get_state()
    except Exception as e:
        print(f"[-] OCR 가동 실패: {e}")
        return
        
    if not cv_state:
        print("[-] 게임 화면 캡처 실패: 'Lineage Classic' 창이 켜져 있고 Stats 패널이 보이는지 확인해주세요.")
        return
        
    level = cv_state.get("level", "0")
    exp = cv_state.get("exp", "0%")
    weight_str = cv_state["weight"].get("text", "0%")
    food_str = cv_state.get("food", "0%")
    
    print(f"    [+] 인식된 화면 정보: LV [{level}] | EXP [{exp}] | WEIGHT [{weight_str}] | FOOD [{food_str}]")
    
    # 숫자 추출
    w_nums = re.findall(r'\d+', weight_str)
    f_nums = re.findall(r'\d+', food_str)
    
    if not w_nums or not f_nums:
        print("[-] 화면에서 무게 및 포만감 수치를 파싱해내지 못했습니다.")
        return
        
    target_w = int(w_nums[0])
    target_f = int(f_nums[0])
    approx_food_val = target_f * 10
    
    print(f"[*] 스캔 대상 타겟 설정: WEIGHT={target_w}%, FOOD={target_f}% ({approx_food_val})")
    ...
    print("[*] 2단계: 힙 메모리에서 실시간 무게 주소 전수 조사 중...")
    ...
    for addr in addr_list:
        try:
            ...
            for off in range(0, 2048 - 4, 4):
                ...
                if abs(val - approx_food_val) <= 15 or abs(val - target_f) <= 1:
                    real_dist = off - 1024
    ...
    print("[*] 3단계: 다단계 포인터 체인(1~3레벨) 추적 엔진 가동...")
